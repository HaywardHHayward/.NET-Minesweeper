            ...
            case ["--h"]:
                Console.WriteLine("""
                                  Minesweeper help:
                                      ...
                                      .../Minesweeper --keyword(s) : Outputs all the keywords used for the different color/interface modes.
            ...
            case ["--keyword"]:
                Console.WriteLine($"""
                                   Minesweeper keywords:
                                       ...
                                       Keyboard mode (both in-game and command line argument): {string.Join(", ",
                                                from word in KeyboardSet.AsParallel()
                                                where word.Split([' ', '-']).Length == 1 select word)}
                                       Keyboard mode (only in-game): {string.Join(", ",
                                                from word in KeyboardSet.AsParallel()
                                                where word.Split([' ', '-']).Length > 1 select word)}
                                       Text mode (both in-game and command line argument): {string.Join(", ",
                                                from word in TextSet.AsParallel()
                                                where word.Split([' ', '-']).Length == 1 select word)}
                                       Text mode (only in-game): {string.Join(", ",
                                                                              from word in KeyboardSet.AsParallel()
                                                                              where word.Split([' ', '-']).Length > 1 select word)}
                                   """);
                return;
